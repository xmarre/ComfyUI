...
def test_native_h3_preserves_existing_controlnet_block_patch(monkeypatch):
    class NativeH3:
        def __init__(self):
            self.blocks = [
                SimpleNamespace(attn=SimpleNamespace(to_gate_compress=None))
    ...
    source.model_options["transformer_options"]["patches_replace"] = {
        "dit": {("double_block", 0): previous}
    }

    patched = _apply(source, vsa=False)

    assert len(patched.replacements) == 1
    replacement, keys = patched.replacements[0]
    assert keys == ("dit", "double_block", 0)
    assert isinstance(replacement, sparse.H3SparseBlockPatch)
    assert replacement.previous is previous


def test_keyless_generic_path_leaves_existing_block_patch_owned_by_previous_node(monkeypatch):
    class NativeH3:
        pass
    ...
    assert patched.replacements == []
    assert (
        patched.model_options["transformer_options"]["patches_replace"]["dit"][
            ("double_block", 0)
        ]
        is previous
    )

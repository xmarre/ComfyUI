...
def test_private_get_immediate_node_signature_fails_closed_for_opaque_input(caching_module, monkeypatch):
    ...
    signature = asyncio.run(
        key_set._get_immediate_node_signature(dynprompt, "node", {}, None)
    )

    assert isinstance(signature, caching.Unhashable)


def test_get_immediate_node_signature_preserves_list_contract(caching_module, monkeypatch):
    caching, nodes_module = caching_module
    monkeypatch.setitem(nodes_module.NODE_CLASS_MAPPINGS, "UnitTestNode", _DummyNode)
    ...
    assert signature[0] == "UnitTestNode"
...
def test_get_immediate_node_signature_preserves_missing_node_contract(caching_module):
    caching, _ = caching_module
    dynprompt = _FakeDynPrompt({})
    key_set = caching.CacheKeySetInputSignature(
        dynprompt,
    ...
    assert isinstance(signature[0], float)
